...
const renderChart = () => {
  ...
  const categoryMap = new Map<string, ChartNode>();
  props.data.forEach((entry) => {
    if (!categoryMap.has(entry.category)) {
      categoryMap.set(entry.category, { name: entry.category, children: [] });
    }
    categoryMap.get(entry.category)!.children!.push({
      name: entry.name,
      value: entry.sumValue,
    });
  });
  chartData.children = Array.from(categoryMap.values());

  // Ersetzen Sie die bestehende Farbskala durch die neue Funktion
  const colorPalette = getMonochromeColorPalette(chartData.children.length + 1);
