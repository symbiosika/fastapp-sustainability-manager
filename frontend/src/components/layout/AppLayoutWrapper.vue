      ...
      <div class="flex flex-col gap-2 mt-5 w-full">
        <div class="flex flex-col gap-4">
          ...
          <div class="flex justify-items-end gap-2">
            ...
            <span class="grow">{{ $t('global.showHelp') }}</span>
    ...
    <template #sidebar>
      <div v-if="route.name === 'onboarding-wizard'">
        <PanelMenu
          :model="[
            {
              label: t('global.sidebar.onBoarding'),
              icon: 'fa-solid fa-home',
              to: '/onboarding-wizard',
            },
            {
              label: t('global.sidebar.skip'),
...
const sidebarActions = [
  ...
  {
    key: 'report-data',
    label: t('global.sidebar.company'),
...
const sidebardata = [
  {
    key: 'equivalents',
    label: t('global.sidebar.equivalents'),
    icon: 'fa-solid fa-list',
    to: '/equivalents',
    visible: true,
  },
...
const sidebar = [
  ...
  {
    key: 'metadata',
    header: t('global.sidebar.metaData'),
    items: sidebardata,
  },
  {
    key: 'csrd',
...
const activeRoute: string = route.name?.toString() ?? '';

const sidebarItemsVisible = ref(<{ [key: string]: boolean }>{
  analysis: true,
  inputs: true,
  actions: ['actions', 'facilities', 'report-data', 'report-targets'].includes(
    activeRoute,
  ),
  metadata: ['equivalents'].includes(activeRoute),
  csrd: true,
});
