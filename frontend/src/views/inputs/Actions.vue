<template>
  ...
  <Dialog
    ...
    modal
    :header="
      selectedAction.id === 'new' ? $t('actions.create') : $t('actions.edit')
    "
    :class="{
  ...
  <ActionList
    ...
    @edit="
      (data: ActionEntry) => {
        selectedAction = data;
        if (selectedAction.finishedUntilIs) {
          selectedAction.finishedUntilIs = new Date(
            selectedAction.finishedUntilIs,
          );
        }
        if (selectedAction.finishedUntilPlanned) {
          selectedAction.finishedUntilPlanned = new Date(
            selectedAction.finishedUntilPlanned,
          );
        }
...
const formEntries: GenericFormEntry[] = [
  ...
  },
  {
    ...
    type: 'select',
    options: [
      { label: t(statusTranslations.open), value: 'open' },
      { label: t(statusTranslations.inProgress), value: 'in_progress' },
      { label: t(statusTranslations.finished), value: 'completed' },
    ],
    optionsKey: 'value',
    optionsLabel: 'label',
  ...
  },
  {
    ...
    type: 'select',
    options: [
      t('actions.measures.increaseEnergyEfficiency'),
      t('actions.measures.expandRenewableEnergy'),
      t('actions.measures.organizationalMeasure'),
      t('actions.measures.compensation'),
    ],
...
const deleteEntry = async (entry: ActionEntry, event: any) => {
  confirm.require({
    ...
    message: t('actions.dialog.deleteEntry'),
